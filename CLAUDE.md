...
| **Auth**          | ✅     | ✅   | ✅ `auth_screen.dart` (sign-in/up sheet) + `_ProfileCard` / `_ProfileSheet` in `main_screen.dart` (More tab profile surface; manage subscription, restore, sign out, deletion request) | —                           |
...
- Firebase Auth and Crashlytics are the only Firebase products in active use — no Firestore. Subscription state lives entirely in RevenueCat (see *Auth ↔ RevenueCat link* below); identity lives in Firebase Auth. There is no server-side user mirror.
...
Pro entitlement is bound to the Firebase uid so purchases follow the user across devices and reinstalls — without this, RevenueCat tracks an anonymous device-uuid customer and a reinstalled-and-signed-in user appears entitlement-less until they manually tap Restore Purchases.

Two integration points:

1. **Cold start** — [RevenueCatService.configure](lib/features/subscription/data/services/revenuecat_service.dart) reads `FirebaseAuth.instance.currentUser?.uid` and passes it as `PurchasesConfiguration.appUserID`. Firebase Auth rehydrates its session before `runApp`, so by the time `runDeferred()` calls `configure()`, the persisted uid is available.
2. **In-session changes** — [RevenueCatAuthLinkService](lib/features/subscription/data/services/revenuecat_auth_link_service.dart) listens to `authRepository.userStream` and calls `Purchases.logIn(uid)` on sign-in, `Purchases.logOut()` on sign-out. It tracks the last-applied uid to short-circuit no-op stream rebuilds. Activated by `revenueCatAuthLinkServiceProvider` in `MuslimCompanionApp.build` so the listener stays alive for the lifetime of the `ProviderContainer`.

Pro gating uses `isProProvider` exclusively (driven by RevenueCat's `CustomerInfo` stream). Sign-up is required only at purchase time — see the auth-gate pattern in `ProPaywallSheet._handleUpgrade`.
...
**Profile surface in `MoreScreen`** — the More tab now has a profile card at the top (`_ProfileCard` in `main_screen.dart`) with two states:

- **Signed out** — tappable CTA that opens `showAuthSheet(...)`. Sign-up is still gated to purchase time inside `ProPaywallSheet`; this is just an additional discoverable entry point.
- **Signed in** — avatar + name + email + live `isProProvider` badge (FREE / PRO). Tapping it opens `_ProfileSheet`, a modal sheet with these actions:
  - **Manage Subscription** — `launchUrl(...)` to `apps.apple.com/account/subscriptions` (iOS) or `play.google.com/store/account/subscriptions?package=com.mdnahid.prayerlock` (Android).
  - **Restore Purchases** — `subscriptionRepositoryProvider.restorePurchases()`, snackbar reflects whether Pro came back.
  - **Sign Out** — `authRepository.signOut()`. The auth stream emission flips the card back to the signed-out CTA; `RevenueCatAuthLinkService` simultaneously logs out of RevenueCat.
  - **Delete Account** — `mailto:thisisnahid78@gmail.com` with a pre-filled subject/body containing the user's email + uid. Matches the deletion path promised in `privacy-policy.md`. Update the constant `_kSupportEmail` (top of `main_screen.dart`) if the support address ever changes — keep it in sync with the privacy policy.

The profile sheet self-closes if `authUserProvider` emits null while it's open (e.g. a Firebase token is revoked mid-session).
